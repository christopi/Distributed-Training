...
class TransVerseModel(nn.Module):
    ...
    def __init__(self, **args):
        ...
        for layer_idx in self.args['model_config']['text_emb_to_img_layers']:
        ...
        self.vd_ckpt_path = self.args['model_config']['video_diffusion']
        self.gen_text_hidden_fcs_video = nn.ModuleList([])
        for layer_idx in self.args['model_config']['text_emb_to_video_layers']:
            if layer_idx == -1 or layer_idx == self.llama_model.config.num_hidden_layers:
        ...
        self.ad_ckpt_path = self.args['model_config']['audio_diffusion']
        self.gen_text_hidden_fcs_audio = nn.ModuleList([])
        for layer_idx in self.args['model_config']['text_emb_to_audio_layers']:
            if layer_idx == -1 or layer_idx == self.llama_model.config.num_hidden_layers:
                in_dim = self.llama_model.config.hidden_size
    ...
    def prompt_wrap(self, img_embeds, input_ids, target_ids, attention_mask):
        ...
        if self.args['model_config']['freeze_lm']:
    ...
    def _dec_align_training_stage_2(self, inputs):
        ...
        if dataset_type == 'TextToImage':
            loss, gen_acc, mse_loss = self._train_with_mode(texts=inputs['output_texts'],
                                                            modality='image',
                                                            num_gen_tokens=self.args['model_config']['num_gen_img_tokens'],
                                                            text_hidden_fcs=self.gen_text_hidden_fcs,
                                                            gen_token_idx=self.args['model_config']['gen_img_token_idx'],
                                                            text_emb_layers=self.args['model_config']['text_emb_to_img_layers'],
                                                            text_prompt_embeddins=inputs['caption_embs'],
                                                            stage=self.stage)
        elif dataset_type == 'TextToVideo':
            loss, gen_acc, mse_loss = self._train_with_mode(texts=inputs['output_texts'],
                                                            modality='video',
                                                            num_gen_tokens=self.args['model_config']['num_gen_video_tokens'],
                                                            text_hidden_fcs=self.gen_text_hidden_fcs_video,
                                                            gen_token_idx=self.args['model_config']['gen_video_token_idx'],
                                                            text_emb_layers=self.args['model_config']['text_emb_to_video_layers'],
                                                            text_prompt_embeddins=inputs['caption_embs'],
                                                            stage=self.stage)
        elif dataset_type == 'TextToAudio':
            loss, gen_acc, mse_loss = self._train_with_mode(texts=inputs['output_texts'],
                                                            modality='audio',
                                                            num_gen_tokens=self.args['model_config']['num_gen_audio_tokens'],
                                                            text_hidden_fcs=self.gen_text_hidden_fcs_audio,
                                                            gen_token_idx=self.args['model_config']['gen_audio_token_idx'],
                                                            text_emb_layers=self.args['model_config']['text_emb_to_audio_layers'],
                                                            text_prompt_embeddins=inputs['caption_embs'],
                                                            stage=self.stage)
        else:
            raise NotImplementedError
    ...
    def generate_tokens_embeddings(self, inputs, input_embeds, temperature: float = 0.0, top_p: float = 1.0):
        ...
        for _hidden_states in outputs.hidden_states[1:]:
            for idx in self.args['model_config']['text_emb_to_img_layers']:
                output_embeddings.append(_hidden_states[idx])
            for idx in self.args['model_config']['text_emb_to_video_layers']:
                video_output_embedding.append(_hidden_states[idx])
            for idx in self.args['model_config']['text_emb_to_audio_layers']:
    ...
    def generate_images(self, generated_ids, embeddings, all_gen_idx, generation_model=None,
        ...
        for gen_idx in all_gen_idx:
            assert generated_ids[0, \
                                 gen_idx:gen_idx + self.args['model_config']['num_gen_img_tokens']]\
                                 .cpu().detach().numpy().tolist() == self.args['gen_img_token_idx'], \
                                 (generated_ids[0, gen_idx:gen_idx + self.args['model_config']['num_gen_img_tokens']], 
                                  self.args['model_config']['gen_img_token_idx'])
            raw_emb = embeddings[:, gen_idx - 1:gen_idx - 1 + self.args['model_config']['num_gen_img_tokens'], :]  # (1, 8, 4096)

            # Produce generation embedding.
            gen_prefix = ' '.join([f'[IMG{i}]' for i in range(self.args['model_config']['num_gen_img_tokens'])])
    ...
    def generate_videos(self, generated_ids, embeddings, all_gen_idx, generation_model=None,
        ...
        for gen_idx in all_gen_idx:
            assert generated_ids[0,
                   gen_idx:gen_idx + self.args['model_config']['num_gen_video_tokens']].cpu().detach().numpy().tolist() == \
                   self.args['model_config']['gen_video_token_idx'],\
                   (generated_ids[0, gen_idx:gen_idx + self.args['model_config']['num_gen_video_tokens']], \
                    self.args['model_config']['gen_video_token_idx'])
            raw_emb = embeddings[:, gen_idx - 1:gen_idx - 1 + self.args['model_config']['num_gen_video_tokens'], :]  # (1, 8, 4096)
            # print(f'gen_idx: {gen_idx}')
            # print('4', raw_emb.size())
            # assert len(self.args['text_emb_to_video_layers']) == 1

            # Produce generation embedding.
            gen_prefix = ' '.join([f'[VID{i}]' for i in range(self.args['model_config']['num_gen_video_tokens'])])
            gen_prefx_ids = self.llama_tokenizer(gen_prefix, add_special_tokens=False,
                                                 return_tensors="pt").input_ids.to(self.device)
            gen_prefix_embs = self.input_embeddings(gen_prefx_ids)  # (1, T_I_V_A.txt, D)
    ...
    def generate_audios(self, generated_ids, embeddings, all_gen_idx, generation_model=None,
        ...
        for gen_idx in all_gen_idx:
            assert generated_ids[0,
                   gen_idx:gen_idx + self.args['model_config']['num_gen_audio_tokens']].cpu().detach().numpy().tolist() == \
                   self.args['model_config']['gen_audio_token_idx'], \
                   (generated_ids[0, gen_idx:gen_idx + self.args['model_config']['num_gen_audio_tokens']], \
                    self.args['model_config']['gen_audio_token_idx'])
            raw_emb = embeddings[:, gen_idx - 1:gen_idx - 1 + self.args['model_config']['num_gen_audio_tokens'], :]  # (1, 8, 4096)
            # print(f'gen_idx: {gen_idx}')
            # print('raw_emb 4', raw_emb.size())
            # assert len(self.args['text_emb_to_video_layers']) == 1

            # Produce generation embedding.
            gen_prefix = ' '.join([f'[AUD{i}]' for i in range(self.args['model_config']['num_gen_audio_tokens'])])
            gen_prefx_ids = self.llama_tokenizer(gen_prefix, add_special_tokens=False,
                                                 return_tensors="pt").input_ids.to(self.device)
            gen_prefix_embs = self.input_embeddings(gen_prefx_ids)  # (1, T_I_V_A.txt, D)
